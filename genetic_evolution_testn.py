
from main import run_network_simulation
from src.neuralnetwork import NeuralNetwork
import numpy as np
import pickle as pkl

class GeneticEvolution2:
    def __init__(self):
        self.learning_rate = [0.0,0.3,0.7,1.0]
        self.record = None

    def bin2int(self,num):
        return num[1]*2+num[0]

    def read_previous_test_file(self,file):
        with open(file, "rb") as f:
            self.record = pkl.load(f)

    def reproduce_members(self):
        probabilities = np.array([c for _,c in self.record])
        probabilities = probabilities/np.sum(probabilities)

        newgenes_index = np.random.choice(range(len(self.record)),size = (100),p=probabilities)
        pairs = np.random.choice(range(len(self.record)),size = (100),replace=False)
        pairs_cat1 = [pairs[i] for i in range(0,len(pairs),2)]
        pairs_cat2 = [pairs[i] for i in range(1,len(pairs),2)]
        crossover_site = np.random.choice(range(self.record[0][0].size),size=(50))
        mutationprob = 0.2
        mutation_no = int(mutationprob*self.record[0][0].size*100)
        mutation_indices = np.sort((np.random.choice(range(self.record[0][0].size*100),size=(mutation_no))),axis=None)
        
        selected_genes = np.array([self.record[i][0].reshape(-1) for i in newgenes_index])
        for i,(p1,p2) in enumerate(zip(pairs_cat1,pairs_cat2)):
            #print(i,p1,p2,crossover_site[i],selected_genes[p1].shape)
            newgene1 = np.concatenate((selected_genes[p1][:crossover_site[i]-1],selected_genes[p2][crossover_site[i]-1:]))
            newgene2 = np.concatenate((selected_genes[p2][:crossover_site[i]-1],selected_genes[p1][crossover_site[i]-1:]))
            selected_genes[p1] = newgene1
            selected_genes[p2] = newgene2
        for mi in mutation_indices:
            index1 = int(mi / 270)
            index2 = int(mi % 270)
            selected_genes[index1][index2] = selected_genes[index1][index2] ^ 1
        
        selected_genes = selected_genes.reshape((100,45,6))
        return selected_genes

    def generate_random_sequence(self,sequence):
        #sequence = np.random.choice([0, 1], size=(45,6))
        str_seq = "".join(str(s) for s in sequence)
        return str_seq,(sequence[0],sequence[1],self.bin2int(sequence[2:4]),self.bin2int(sequence[4:6]))
    
    def decode_single_sequence(self,sequence):
        learning_rate = np.zeros((45))
        learning_rule = np.zeros((45),dtype=int)
        driving_rule = np.zeros((45),dtype=int)
        excitatory_rule = np.zeros((45),dtype=int)
        final_seq = ""
        for i in range(45):
            seq,seq_data = self.generate_random_sequence(sequence[i])
            final_seq += seq
            learning_rate[i] = self.learning_rate[seq_data[3]]
            learning_rule[i] = seq_data[2]
            driving_rule[i] = seq_data[0]
            excitatory_rule[i] = seq_data[1] if seq_data[1] == 1 else -1
        return sequence,final_seq,driving_rule,excitatory_rule,learning_rate,learning_rule

    def run_test_evolutionn(self):
        newgenes = self.reproduce_members()
        number_sensors = 12
        population_size = 100
        data = []
        datapkl = []
        for p in range(population_size):
            nseq,seq,driving_rule,excitatory_rule,learning_rate,learning_rule = self.decode_single_sequence(newgenes[p])
            nn = NeuralNetwork(number_sensors,driving_rule,excitatory_rule,learning_rate,learning_rule)
            res = run_network_simulation(delta_t=1,
                           graphGUI=None,
                           track=False,
                           num_landmarks=0,
                           max_time_steps = 80,
                           network = nn)
            data.append((seq,res))
            datapkl.append((nseq,res))
            print (p," population running ", end='\r')
        print ("")
        with open(f'./src/experiments_data/genetic_evolution_results1.txt', 'w') as f:
            for s,r in data:
                f.write(f"{s},{r}\n")
        with open(f'./src/experiments_data/genetic_evolution_resultsb1.pkl', 'wb') as f:
            pkl.dump(datapkl, f)       

if __name__ == "__main__":
    ge = GeneticEvolution2()
    ge.read_previous_test_file('./src/experiments_data/genetic_evolution_resultsb.pkl')
    ge.run_test_evolutionn()