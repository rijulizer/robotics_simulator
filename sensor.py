...
class SensorLine:
    def __init__(self,pos_x:float,pos_y:float,radius:float,robot_theta:float,theta:float,sensorlength:float,i:int):
    ...
    def get_sensorpt(self,robot_pt:list,line_pt1:np.ndarray,line_pt2:np.ndarray):
        ...
        """        
        sensorline_pt1 = np.array([robot_pt[0] + self.radius*np.cos(self.ftheta),robot_pt[1] + self.radius*np.sin(self.ftheta)])
        sensorline_pt2 = np.array([robot_pt[0] + (self.radius+self.sensor_length)*np.cos(self.ftheta),robot_pt[1] + (self.radius+self.sensor_length)*np.sin(self.ftheta)])
    ...
    def draw(self, surface):
        self.body = pygame.draw.line(surface,(0,0,0),(self.pos_x + self.radius*np.cos(self.ftheta),self.pos_y + self.radius*np.sin(self.ftheta)),(self.pos_x + (self.radius+self.sensor_length)*np.cos(self.ftheta),self.pos_y + (self.radius+self.sensor_length)*np.sin(self.ftheta)),1)      
                                                 
class SensorManager:
    def __init__(self,pos_x:float,pos_y:float,radius:float,theta:float,no_sensor:int,sensorlength:int,environment:Environment):
        ...
        self.text_surface = [self.pygame_font.render(self.sensor_read[i], False, (0,0,0)) for i in range(self.no_sensor)]
        self.sensors = []
    ...
    def draw(self, surface):
        ...
        for i in range(self.no_sensor):
            surface.blit(self.text_surface[i],((self.pos_x + (self.radius+self.sensor_length+self.deltalist[i])*np.cos(self.theta+self.sensor_points_rad[i])),
                                                 (self.pos_y + (self.radius+self.sensor_length+self.deltalist[i])*np.sin(self.theta + self.sensor_points_rad[i]))))