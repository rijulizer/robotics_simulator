...
class SensorLine:
    def __init__(self,
        ...
        """
        self.start_x = agent_stats["pos_x"]
        self.start_y = agent_stats["pos_y"]
    ...
    def update(self,
        ...
        if dist <= 0:
            ...
            # the sensor line till the point of intersection
            x2 = self.pos_x + (self.radius + dist) * np.cos(self.f_theta)
            y2 = self.pos_y + (self.radius + dist) * np.sin(self.f_theta)
...
class SensorManager:
    def __init__(self,
        ...
        for i in range(self.no_sensor):
            ...
            # initialize the end points of the sensor line
            sl.update(agent_stats)
        ...
        for line in self.object_list:
            for sensor_line in self.sensors:
                if line.body.colliderect(sensor_line.body):
                    ...
                    sensor_line.update_end_points(dist)
    ...
    def draw(self,
        ...
        self.update_agent_sensor_info(agent_stats)
        # self.update_sensor_status()
        for i in range(self.no_sensor):
            # get end points of sensor lines
            [(x1, y1), (x2, y2)] = self.sensors[i].draw_points
        self.update_sensor_status()
        for i in range(self.no_sensor):
            self.sensors[i].body = pygame.draw.line(surface, (0, 0, 0), (x1, y1), (x2, y2), width=int(self.sensors[i].radius / 10))
